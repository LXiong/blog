...
VI/VIM 是程序员最常用的工具之一，尤其是 unix/linux 上；而查找替换也是最频繁的操作之一。

## 问题

在图形化编辑器中我们经常使用的查找功能在 vi/vim 中能实现吗？

答案是肯定的，并且功能绝对会让你满意。

接下来我们将会解决如下问题：

- 将当前行的第一个匹配到的 `hello` 替换为 `hi`
- 将当前行的所有匹配到的 `hello` 替换为 `hi`
- 将所有行的第一个匹配到的 `hello` 替换为 `hi`
- 将所有行的所有匹配到的 `hello` 替换为 `hi`
- 将指定行范围内第一个匹配到的 `hello` 替换为 `hi`
- 将指定行范围内所有匹配到的 `hello` 替换为 `hi`
- 指定特定字符作为替换范围进行替换
- 准确的匹配单词 `hello` 并将其替换为 `hi`
- 对特殊字符进行替换，如：空格，换行之类
- 简单的正则匹配，包括：重复性模式，重复次数限制，多选模式
- 使用其他分隔符代替常用的 `/` 分隔
- 区域选择及操作

还有其他问题吗？我想到的就这么多，下面我们一一测试。
...
其他的标记（flags）包括：p（print），是要求打印所做的改动；c（confirm），是要求在做出改动以前先询问；i（ignorecase），是不区分大小写。我们可以组合使用标记。
...
这里多说一点，你还可以使用 `#` 或 `+` 作为分隔符，那么 `/` 将被保留了。
